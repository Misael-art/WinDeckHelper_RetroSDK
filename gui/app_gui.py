...
class AppGUI(tk.Tk):
    ...
    def _start_installation_thread(self):
        """Inicia a instalação usando o dashboard aprimorado."""
        ...
        # Usar o dashboard aprimorado em vez do diálogo simples
        try:
            from gui.realtime_installation_dashboard import create_installation_dashboard
            
            # Criar e mostrar o dashboard de instalação
            dashboard = create_installation_dashboard(self)
            dashboard.start_installations(selected_components, self.components_data)
            
        except ImportError:
            # Fallback para o método antigo se o dashboard não estiver disponível
            self._start_legacy_installation(selected_components)
    
    def _start_legacy_installation(self, selected_components):
        """Método de instalação legado como fallback."""
