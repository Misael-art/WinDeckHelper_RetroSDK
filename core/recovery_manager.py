...
from env_dev.core.version_manager import version_manager
...
class RecoveryManager:
    ...
    def _compare_versions(self, current: str, latest: str) -> bool:
        ...
        try:
            ...
            # Usa o VersionManager centralizado para comparação
            comparison = version_manager.compare_versions(current, latest)
            return comparison < 0  # True se current < latest (precisa atualizar)
        except Exception as e:
            logger.warning(f"Erro ao comparar versões {current} vs {latest}: {e}")
            return False
