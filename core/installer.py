...
# Importa o wrapper de compatibilidade
from .installer_wrapper import InstallerWrapper

# Classe Installer para compatibilidade
class Installer(InstallerWrapper):
    """
    Classe Installer para compatibilidade com código legado.
    Herda de InstallerWrapper que mantém a interface antiga.
    """
    pass
...
from utils import downloader, extractor, installer_runner, env_manager, hash_utils
from utils import env_checker # Importa o novo módulo de verificação
from utils import disk_space, permission_checker # Importa os novos módulos
...
from utils.log_manager import setup_logging, INSTALLATION, ROLLBACK, VERIFICATION, SUCCESS
...
def _install_java_fallback(component_name, component_data, rollback_mgr: RollbackManager, status_queue=None):
    """Instalação alternativa do Java quando Chocolatey falha."""
    try:
        logger.info("Tentando instalação alternativa do Java via download direto...")
        send_status_update({
            'type': 'stage',
            'component': component_name,
            'stage': 'Instalação alternativa do Java'
        }, status_queue=status_queue)
        
        # URL do OpenJDK 21 para Windows
        java_url = "https://download.java.net/java/GA/jdk21.0.1/415e3f918a1f4062a0074a2794853d0d/12/GPL/openjdk-21.0.1_windows-x64_bin.zip"
        
        # Diretório de destino
        java_install_dir = Path("C:/Program Files/Java/openjdk-21")
        ...
        with tempfile.NamedTemporaryFile(suffix=".zip", delete=False) as temp_file:
            logger.info(f"Baixando Java de {java_url}...")
            urllib.request.urlretrieve(java_url, temp_file.name)
            
            # Extrair para o diretório de instalação
            logger.info(f"Extraindo Java para {java_install_dir}...")
            java_install_dir.mkdir(parents=True, exist_ok=True)
            
            with zipfile.ZipFile(temp_file.name, 'r') as zip_ref:
                zip_ref.extractall(java_install_dir.parent)
            
            # Renomear diretório extraído para o nome padrão
            extracted_dirs = list(java_install_dir.parent.glob("jdk-21*"))
            if extracted_dirs:
                extracted_dir = extracted_dirs[0]
                if extracted_dir != java_install_dir:
                    if java_install_dir.exists():
                        shutil.rmtree(java_install_dir)
                    extracted_dir.rename(java_install_dir)
            
            # Configurar variáveis de ambiente
            java_home = str(java_install_dir)
            java_bin = str(java_install_dir / "bin")
            ...
            try:
                with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, 
                    ...
                    try:
                        ...
                        winreg.SetValueEx(key, "PATH", 0, winreg.REG_EXPAND_SZ, java_bin)
                        
                logger.info("Variáveis de ambiente configuradas com sucesso")
            except Exception as e:
                logger.warning(f"Falha ao configurar variáveis de ambiente: {e}")
            
            # Limpar arquivo temporário
            os.unlink(temp_file.name)
            
            # Registrar para rollback
            rollback_mgr.add_action(
                'remove_directory',
                {'path': str(java_install_dir), 'component_name': component_name}
            )
            
            logger.info("Java instalado com sucesso via método alternativo")
            send_status_update({
                'type': 'result',
                'component': component_name,
                'status': 'SUCCESS',
                'message': 'Java instalado via método alternativo'
            }, status_queue=status_queue)
            
            return True
            
    except Exception as e:
        logger.error(f"Falha na instalação alternativa do Java: {e}")
        send_status_update({
            'type': 'result',
            'component': component_name,
            'status': 'FAILED',
            'message': f'Falha na instalação alternativa: {str(e)}'
        }, status_queue=status_queue)
        return False


def _install_chocolatey(component_name, component_data, rollback_mgr: RollbackManager, status_queue=None):
    """Instala um componente usando Chocolatey."""
    package_name = component_data.get('package_name')
    if not package_name:
        err = installation_error(f"'package_name' não definido para '{component_name}'.", component=component_name, severity=ErrorSeverity.ERROR)
        err.log()
        send_status_update({
            'type': 'result',
            'component': component_name,
            'status': 'FAILED',
            'message': 'Nome do pacote Chocolatey não especificado'
        }, status_queue=status_queue)
        return False

    try:
        # Verifica se o Chocolatey está instalado
        choco_check = subprocess.run(['choco', '--version'], capture_output=True, text=True)
        if choco_check.returncode != 0:
            logger.error(f"Chocolatey não está instalado. Necessário para instalar '{component_name}'")
            send_status_update({
                'type': 'result',
                'component': component_name,
                'status': 'FAILED',
                'message': 'Chocolatey não está instalado'
            }, status_queue=status_queue)
            return False

        logger.info(f"Instalando '{component_name}' via Chocolatey: {package_name}")
        send_status_update({
            'type': 'stage',
            'component': component_name,
            'stage': f'Instalando via Chocolatey: {package_name}'
        }, status_queue=status_queue)

        # Comando de instalação
        install_cmd = ['choco', 'install', package_name, '-y']
        
        # Adiciona argumentos extras se especificados
        extra_args = component_data.get('install_args', [])
        if extra_args:
        ...
        if result.returncode == 0:
            logger.info(f"'{component_name}' instalado com sucesso via Chocolatey")
            
            # Executa comandos pós-instalação se especificados
            post_install_commands = component_data.get('post_install_commands', [])
            for cmd in post_install_commands:
                try:
                    logger.info(f"Executando comando pós-instalação: {cmd}")
                    subprocess.run(cmd, shell=True, check=True, timeout=300)
                except subprocess.CalledProcessError as e:
                    logger.warning(f"Comando pós-instalação falhou: {cmd} - {e}")
                except subprocess.TimeoutExpired:
                    logger.warning(f"Comando pós-instalação expirou: {cmd}")
            
            # Registra para rollback
            rollback_mgr.add_action(
                'chocolatey_uninstall',
                {'package_name': package_name, 'component_name': component_name}
            )
            ...
            send_status_update({
                ...
                'component': component_name,
                'status': 'SUCCESS',
                'message': f'Instalado via Chocolatey: {package_name}'
            }, status_queue=status_queue)
            
            return True
        else:
            logger.error(f"Falha ao instalar '{component_name}' via Chocolatey: {result.stderr}")
            
            # Fallback para Java Runtime Environment
            if component_name == 'Java Runtime Environment' and 'temurin' in package_name:
                logger.info(f"Tentando instalação alternativa do Java...")
                if _install_java_fallback(component_name, component_data, rollback_mgr, status_queue):
                    return True
            ...
            send_status_update({
                ...
                'component': component_name,
                'status': 'FAILED',
                'message': f'Falha na instalação Chocolatey: {result.stderr}'
            }, status_queue=status_queue)
            return False
            
    except subprocess.TimeoutExpired:
        logger.error(f"Timeout na instalação de '{component_name}' via Chocolatey")
        send_status_update({
            'type': 'result',
            'component': component_name,
            'status': 'FAILED',
            'message': 'Timeout na instalação Chocolatey'
        }, status_queue=status_queue)
        return False
    except Exception as e:
        logger.error(f"Erro ao instalar '{component_name}' via Chocolatey: {e}")
        send_status_update({
            'type': 'result',
            'component': component_name,
            'status': 'FAILED',
            'message': f'Erro na instalação Chocolatey: {str(e)}'
        }, status_queue=status_queue)
        return False
...
def _verify_installation(component_name, component_data):
    ...
    for group_type, group_actions in actions_by_type.items():
        group_passed = False
        group_results = []
        valid_checks_in_group = 0
        
        logger.verification(f"Verificando grupo '{group_type}' com {len(group_actions)} ações (lógica OR)")
            ...
            try:
                if action_type == 'file_exists':
                    ...
                    try:
                        ...
                        check_details['path'] = registry_path
                        
                        if not registry_path:
                            error_msg += ": Caminho do registro não especificado"
                        else:
                            # Dividir o caminho do registro
                            parts = registry_path.split('\\', 1)
                            if len(parts) != 2:
                                error_msg += f": Formato de caminho de registro inválido: {registry_path}"
                            else:
                                ...
                                if root_key is None:
                                    error_msg += f": Chave raiz do registro desconhecida: {root_key_name}"
                                else:
                                    # Tentar abrir a chave do registro
                                    try:
                                        with winreg.OpenKey(root_key, subkey_path, 0, winreg.KEY_READ):
                                            check_passed = True
                                            check_details['found'] = True
                                    except FileNotFoundError:
                                        error_msg += f": Registro não encontrado: {registry_path}"
                                        check_details['found'] = False
                                    except PermissionError:
                                        error_msg += f": Sem permissão para acessar registro: {registry_path}"
                                        check_details['found'] = False
                                        
                    except ImportError:
                        error_msg += ": Módulo winreg não disponível (não é Windows?)"
                        check_details['error'] = 'winreg_not_available'
                    except Exception as e:
                        error_msg += f": Erro ao verificar registro: {e}"
                        check_details['error'] = str(e)

                else:
                    logger.verification(f"Tipo de verificação desconhecido '{action_type}' para '{component_name}'.")
                    # Registra o erro de verificação para tipo desconhecido
                    group_results.append({
                        'type': action_type,
                        'passed': False,
                        'error': f"Tipo de verificação desconhecido '{action_type}'",
                        'details': {'action': action}
                    })
                    continue
                
                # Incrementa contador de verificações válidas (apenas para tipos conhecidos)
                valid_checks_in_group += 1
                ...
                group_results.append(verification_result)

                if check_passed:
                    logger.verification(f"Verificação {i+1}/{len(group_actions)} ({action_type}) para '{component_name}' passou.")
                    group_passed = True  # Pelo menos uma passou no grupo
                else:
        ...
        # Adiciona os resultados do grupo aos resultados gerais
        verification_results.extend(group_results)
        
        # Verifica se o grupo passou (lógica OR)
        # Se não há verificações válidas no grupo, considera como falha
        if valid_checks_in_group == 0:
            logger.verification(f"Grupo '{group_type}' falhou - nenhuma verificação válida encontrada")
            all_groups_passed = False
        elif not group_passed:
            logger.verification(f"Grupo '{group_type}' falhou - nenhuma verificação passou")
            all_groups_passed = False
            
            # Notifica erro de verificação apenas se o grupo inteiro falhou
            send_status_update({
                'type': 'error',
                'component': component_name,
                'category': 'VERIFICATION',
                'message': f"Grupo de verificação '{group_type}' falhou",
                'severity': 'ERROR',
                'recoverable': True
            }, status_queue=status_queue)
        else:
            logger.verification(f"Grupo '{group_type}' passou - pelo menos uma verificação foi bem-sucedida")
    ...
    verification_status = 'SUCCESS' if all_groups_passed else 'FAILED'
    send_status_update({
    ...
    if all_groups_passed:
        logger.verification(f"Verificação de '{component_name}' passou - todos os grupos de verificação foram bem-sucedidos (lógica OR aplicada).")
    else:
        logger.verification(f"Verificação de '{component_name}' falhou - um ou mais grupos de verificação falharam.")
        logger.rollback(f"Preparando para rollback de '{component_name}' devido a falhas nas verificações.")

    return all_groups_passed
...
def _install_python_module(component_name, component_data, status_queue=None):
    """Trata instalações de módulos Python"""
    try:
        logger.info(f"Executando instalação de módulo Python para '{component_name}'")
        
        # Para módulos Python, geralmente não há instalação física
        # Apenas verificamos se o módulo pode ser importado
        module_path = component_data.get('module_path')
        if module_path:
            try:
                # Tenta importar o módulo para verificar se está disponível
                import importlib
                importlib.import_module(module_path)
                logger.info(f"Módulo Python '{module_path}' está disponível")
            except ImportError:
                logger.warning(f"Módulo Python '{module_path}' não está disponível, mas continuando...")
        
        # Módulos Python são considerados "instalados" se o código está presente
        logger.info(f"Instalação de módulo Python '{component_name}' concluída")
        
        send_status_update({
            'type': 'result',
            'component': component_name,
            'status': 'SUCCESS',
            'message': f"Módulo Python '{component_name}' instalado com sucesso"
        }, status_queue=status_queue)
        
        return True
        
    except Exception as e:
        logger.error(f"Erro na instalação do módulo Python '{component_name}': {e}")
        send_status_update({
            'type': 'result',
            'component': component_name,
            'status': 'FAILED',
            'message': f"Erro na instalação do módulo Python: {e}"
        }, status_queue=status_queue)
        return False

def _handle_custom_installation_legacy(component_name, component_data, status_queue=None):
    """Trata instalações customizadas na função legacy"""
    try:
        custom_installer = component_data.get('custom_installer')
        if not custom_installer:
            logger.error(f"Instalador customizado não especificado para '{component_name}'")
            return False
        
        logger.info(f"Executando instalador customizado: {custom_installer}")
        
        if '.' in custom_installer:
            module_name, function_name = custom_installer.rsplit('.', 1)
            
            # Importar e executar função do retro_devkit_manager
            if module_name == 'retro_devkit_manager':
                try:
                    ...
                    if hasattr(manager, function_name):
                        installer_func = getattr(manager, function_name)
                        result = installer_func()
                        
                        if result:
                            logger.info(f"✅ Instalação customizada de '{component_name}' concluída com sucesso")
                            send_status_update({
                                'type': 'result',
                                'component': component_name,
                                'status': 'SUCCESS',
                                'message': 'Instalação customizada concluída'
                            }, status_queue=status_queue)
                        else:
                            logger.error(f"❌ Falha na instalação customizada de '{component_name}'")
                            send_status_update({
                                'type': 'result',
                                'component': component_name,
                                'status': 'FAILED',
                                'message': 'Falha na instalação customizada'
                            }, status_queue=status_queue)
                        
                        return result
                    else:
                        logger.error(f"Função '{function_name}' não encontrada no RetroDevKitManager")
                        return False
                        
                except ImportError as e:
                    logger.error(f"Erro ao importar RetroDevKitManager: {e}")
                    return False
                ...
                try:
                    ...
                    if hasattr(installer, function_name):
                        installer_func = getattr(installer, function_name)
                        result = installer_func()
                        ...
                        logger.error(f"Função '{function_name}' não encontrada no IntelligentSGDKInstaller")
                        return False
                        
                except ImportError as e:
                    logger.error(f"Erro ao importar IntelligentSGDKInstaller: {e}")
                    return False
            else:
                logger.warning(f"Instalador customizado não suportado: {custom_installer}")
                return False
        else:
            logger.error(f"Formato de instalador customizado inválido: {custom_installer}")
            return False
            
    except Exception as e:
        logger.error(f"Erro no instalador customizado para '{component_name}': {e}")
        return False
...
class InstallationActionManager:
    ...
    def _perform_installation(self) -> bool:
        ...
        if self.install_method in ['download', 'archive', 'exe', 'msi']:
            ...
            return self._handle_custom_installation()

        elif self.install_method == 'python_module':
            return self._handle_python_module_installation()
    ...
    def _handle_python_module_installation(self) -> bool:
        """Trata instalações de módulos Python"""
        try:
            self.logger.info(f"Executando instalação de módulo Python para '{self.component_name}'")
            
            # Para módulos Python, geralmente não há instalação física
            # Apenas verificamos se o módulo pode ser importado
            module_path = self.component_data.get('module_path')
            if module_path:
                try:
                    # Tenta importar o módulo para verificar se está disponível
                    import importlib
                    importlib.import_module(module_path)
                    self.logger.info(f"Módulo Python '{module_path}' está disponível")
                except ImportError:
                    self.logger.warning(f"Módulo Python '{module_path}' não está disponível, mas continuando...")
            
            # Módulos Python são considerados "instalados" se o código está presente
            self.logger.info(f"Instalação de módulo Python '{self.component_name}' concluída")
            return True
            
        except Exception as e:
            self.logger.error(f"Erro na instalação do módulo Python '{self.component_name}': {e}")
            return False

    def _handle_custom_installation(self) -> bool:
        """Trata instalações customizadas usando instaladores específicos"""
        try:
            custom_installer = self.component_data.get('custom_installer')
            if not custom_installer:
                self.logger.error(f"Instalador customizado não especificado para '{self.component_name}'")
                return False
            
            self.logger.info(f"Executando instalador customizado: {custom_installer}")
                ...
                if module_name == 'retro_devkit_manager':
                    try:
                        ...
                        if hasattr(manager, function_name):
                            ...
                            if result:
                                self.logger.info(f"✅ Instalação customizada de '{self.component_name}' concluída com sucesso")
                            else:
                                self.logger.error(f"❌ Falha na instalação customizada de '{self.component_name}'")
                            
                            return result
                        else:
                            self.logger.error(f"Função '{function_name}' não encontrada no RetroDevKitManager")
                            return False
                            
                    except ImportError as e:
                        self.logger.error(f"Erro ao importar RetroDevKitManager: {e}")
                        return False
                elif module_name == 'intelligent_sgdk_installer':
                    try:
                        from .intelligent_sgdk_installer import get_intelligent_sgdk_installer
                        ...
                        if hasattr(installer, function_name):
                            ...
                            self.logger.error(f"Função '{function_name}' não encontrada no IntelligentSGDKInstaller")
                            return False
                            
                    except ImportError as e:
                        self.logger.error(f"Erro ao importar IntelligentSGDKInstaller: {e}")
                        return False
                else:
                    self.logger.warning(f"Instalador customizado não suportado: {custom_installer}")
                    return False
            else:
                self.logger.error(f"Formato de instalador customizado inválido: {custom_installer}")
                return False
                
        except Exception as e:
            self.logger.error(f"Erro no instalador customizado para '{self.component_name}': {e}")
            return False
...
def install_component(component_name, component_data, all_components_data=None, installed_components=None, progress_callback=None, rollback_mgr=None, status_queue=None):
    ...
    try:
        ...
        try:
            if install_method in ['download', 'archive', 'exe', 'msi']:
            ...
            elif install_method == 'chocolatey':
                success = _install_chocolatey(
                    component_name,
                    component_data,
                    rollback_mgr,
                    status_queue=status_queue
                ...
                success = _handle_custom_installation_legacy(
                    component_name,
                    component_data,
                    status_queue=status_queue
                )
            elif install_method == 'python_module':
                success = _install_python_module(
                    component_name,
                    component_data,
                    status_queue=status_queue
                )
